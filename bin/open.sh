...
logcat -c
DBG=1 APC=1 APCPORT="$APCPORT" NOPRE="${NOPRE:-0}" POKE="$POKE" GOT=1 AUTO=1 TRANSACT=1 STREAM=1 CMD=2:0 CW=8 S=3000 \
...
grep -a -E 'ACP3|NOPRE|PFD@|CMD:|reply=' /data/local/tmp/stop.log | head -20
echo "== HAL 原话"
