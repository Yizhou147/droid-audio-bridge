...
#define S(field, name) do { *(void**)&B.field = dlsym(h, name); \
  ...
  S(WaitForService, "AServiceManager_waitForService");
...
int main(int argc, char** argv) {
  ...
  if (load()) return 2;
  AIBinder* mod = B.WaitForService(svc);
  if (!mod) { fprintf(stderr, "STEP1-FAIL waitForService(%s)：SELinux 或 servicemanager 挡了\n", svc); return 3; }
